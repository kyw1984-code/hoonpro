...
test('percent 쿠폰은 정가 비율로 깎는다', () => {
  assert.equal(couponDiscount({ type: 'percent', value: 10 }, MONTHLY, 1), 3980);
  assert.equal(couponDiscount({ type: 'percent', value: 10 }, YEARLY, 12), 35760);
});
...
test('무료 일수와 할인은 한 쿠폰에 함께 담긴다', () => {
  ...
  assert.equal(applyDiscounts(MONTHLY, c, 1).amount, 34800);
  assert.equal(applyDiscounts(YEARLY, c, 12).amount, 297600);
...
test('추천 보상은 추천인의 플랜과 무관하게 월간 정가 기준 금액이다', () => {
  ...
  assert.equal(reward, 3980);
  // 월간 추천인
  assert.equal(applyDiscounts(MONTHLY, null, 1, reward).amount, 35820);
  // 연간 추천인 — 같은 금액만 깎인다 (비율이었다면 35,760원이 나갔다)
  assert.equal(applyDiscounts(YEARLY, null, 12, reward).amount, 353620);
});

test('쿠폰 할인과 추천 보상은 함께 적용된다', () => {
  const c = { type: 'amount_monthly', value: 5000 };
  const r = applyDiscounts(MONTHLY, c, 1, 3980);
  assert.equal(r.fromCoupon, 5000);
  assert.equal(r.fromReward, 3980);
  assert.equal(r.discount, 8980);
  assert.equal(r.amount, MONTHLY - 8980);
});
