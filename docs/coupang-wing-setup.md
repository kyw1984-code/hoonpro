...
`vercel.json`에 세 개가 추가돼 있다. **Vercel Pro가 필요하다.**

Hobby는 크론 2개에 하루 1회가 상한이라, 이 설정이 들어간 커밋은 빌드가 시작되기도
전에 배포가 거절된다. 코드 오류처럼 보이지만 원인은 요금제다. 프로젝트 전체가
Pro여야 매시 수집이 동작한다.

> Hobby 상태에서 이 브랜치를 푸시하면 GitHub 커밋 상태에 "Vercel – Deployment failed"만 찍히고
> Vercel 대시보드에는 배포 기록 자체가 남지 않는다. 빌드 로그도 없다. 이 증상은 코드 오류가
> 아니라 요금제 한도(매시 크론·크론 5개)에서 배포가 생성 전에 거절된 것이다. Pro로 올린 뒤
> 새 커밋을 푸시하면 정상적으로 빌드된다.
...
5. 훈프로 [정산AI] 탭에서 업체코드·Access Key·Secret Key 입력
