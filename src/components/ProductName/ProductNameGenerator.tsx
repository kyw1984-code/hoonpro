      ...
      <div className="bg-paper rounded-card border border-line p-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-ink mb-1">브랜드 <span className="text-ink-3 font-normal">(없으면 공란)</span></label>
              <input
                type="text"
                value={brand}
                onChange={e => setBrand(e.target.value)}
                placeholder="예: 나이키, 훈프로"
                className="w-full p-3 border border-line-strong rounded-card focus:ring-2 focus:ring-orange-400 outline-none"
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-ink mb-1">타겟 (성별/대상)</label>
              <select
                value={target}
                onChange={e => setTarget(e.target.value)}
                className="w-full p-3 border border-line-strong rounded-card focus:ring-2 focus:ring-orange-400 outline-none bg-paper"
              >
            ...
            <div>
              <label className="block text-sm font-medium text-ink mb-2">시즌 (여러개 선택 가능)</label>
          ...
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-ink mb-1">제품명 1 (핵심 키워드) <span className="text-critical">*필수</span></label>
              <input
                type="text"
                value={mainKeyword}
            ...
            <div>
              <label className="block text-sm font-medium text-ink mb-1">소구점 (특징/재질/핏)</label>
              <input
                type="text"
                value={appealPoint}
            ...
            <div>
              <label className="block text-sm font-medium text-ink mb-1">제품명 2 (세부 키워드)</label>
              <input
                type="text"
                value={subKeyword}
            ...
            <div>
              <label className="block text-sm font-medium text-ink mb-1">구성 (몇종/세트)</label>
              <input
                type="text"
                value={setInfo}
        ...
        <div className="mt-8 border-t border-line pt-8">
          ...
          {mainKeyword ? (
            <div className="space-y-4">
              {/* 상품명 출력 */}
              <div>
                <p className="text-sm font-medium text-ink-2 mb-1">✅ 최종 상품명</p>
                <p className="text-xs text-ink-3 mb-2">공식: 브랜드 + 타겟 + 시즌 + 제품명1 + 소구점 + 제품명2 + 구성</p>
                <div className="relative bg-paper-2 border border-line rounded-card p-4 pr-16">
                  <p className="text-ink font-medium text-lg break-all">{finalTitle}</p>
                  <button
                    onClick={handleCopy}
                    className="absolute top-3 right-3 p-2 rounded-control bg-paper border border-line hover:bg-orange-50 hover:border-orange-400 transition-all"
                  >
                    {copied
                ...
                <p className="text-xs text-ink-3 mt-1">📏 글자수: {textLen}자 (공백 포함)</p>
              </div>
            ...
            <div className="bg-paper-2 border border-line rounded-card p-6 text-center text-ink-3">
              👆 위 칸에 '제품명 1'을 입력하세요.
            </div>
          )}
        </div>
