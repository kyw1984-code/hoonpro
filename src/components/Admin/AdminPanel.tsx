    ...
    <div className="max-w-5xl mx-auto px-6 py-8">
      ...
      <div className="mb-6 flex gap-1 overflow-x-auto border-b border-line">
        ...
        <button
          ...
          className={`flex shrink-0 items-center gap-1.5 whitespace-nowrap px-4 py-2.5 text-sm font-medium border-b-2 transition-colors ${
            tab === 'limits' ? 'border-accent text-accent' : 'border-transparent text-ink-2 hover:text-ink'
          }`}
        >
          <SlidersHorizontal className="w-4 h-4" /> 사용 한도
        </button>
        <button
...
const TAB_LABELS: { id: string; label: string }[] = [
  ...
  { id: 'sourcing', label: '소싱AI' },
  { id: 'ranktracker', label: '순위추적AI' },
  { id: 'review', label: '리뷰 분석AI' },
  { id: 'analyzer', label: '광고분석AI' },
  { id: 'coupang', label: '정산AI' },
...
const FEATURE_LABELS: Array<{ id: string; label: string }> = [
  ...
  { id: 'ranktracker.changes', label: '순위추적AI › 경쟁 상품 변동(7일)' },
  { id: 'coupang.experiments', label: '정산AI › 변경 효과 측정 (실험 노트)' },
  { id: 'coupang.purchases', label: '정산AI › 1688 매입 원가 계산' },
];
