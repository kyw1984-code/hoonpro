  ...
  return (
    <div className="flex flex-col gap-5">
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        ...
        />
        <Stat label="일정 미배정" value={won(data.totals.unscheduled)} sub="윙 기준 · 지급일이 아직 안 잡힌 정산예정액 (어림)" />
      </div>
      ...
      {data.totals.unscheduled > 0 && (
        <p className="rounded-card border border-line bg-paper-2 px-3 py-2 text-[11.5px] leading-relaxed text-ink-3">
          <b className="text-ink-2">'일정 미배정'은 어림이고, 판매자배송(윙)만 셉니다.</b> 쿠팡 지급내역 API에는
          로켓그로스가 들어오지 않아 그로스를 넣으면 그로스 매출 전액이 매달 '미배정'으로 잡힙니다.
          윙 안에서도 어림입니다 — 우리는 <b>판매일</b> 기준으로 세고 쿠팡은 <b>구매확정일</b> 기준으로 인식하기 때문입니다.
          8월에 판 것이 9월에 구매확정되면 쿠팡 장부에서는 9월 매출이라, 최근 달일수록 이 숫자가 크게 나옵니다.
