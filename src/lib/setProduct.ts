 ...
 * 1+1, 2+1 같은 덤 표기.
 *
 * 앞뒤로 숫자나 소수점이 붙으면 잡지 않는다. \b만 두면 '1.5 + 1.5kg'에서
 * 가운데의 '5 + 1'이 걸려 6개들이로 읽힌다. 45,000원짜리 오겹살이 낱개
 * 7,500원이 되고, 그 값이 가격 적합도·시장 평균가·내 가게 기준 점수로
 * 그대로 흘러간다. 틀렸다는 걸 알아챌 방법이 없는 종류의 오류다.
 */
const PLUS = /(?<![\d.])(\d+)\s*\+\s*(\d+)(?![\d.])/;
