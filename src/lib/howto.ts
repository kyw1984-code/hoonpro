...
export const HOWTO: Record<string, HowToGuide> = {
  ...
  'coupang.health': {
    ...
    steps: [
      ...
      { title: '바로 처리', desc: '각 묶음의 버튼을 누르면 원가 입력·가격 관리·재고 예측·반품 분석으로 갑니다.' },
    ],
    caution: '판정은 우리가 수집한 데이터 기준입니다. 수집 직후가 아니면 재고는 최대 하루 전 값입니다.',
  },
  ...
  'coupang.hours': {
    ...
    steps: [
      ...
      { title: '기간 바꾸기', desc: '최근 14·28·56·90일 중 고릅니다. 시즌이 바뀌면 패턴도 바뀝니다.' },
    ],
    caution: '수집 때부터 쌓이는 값이라 처음 며칠은 비어 보입니다. 윙·로켓그로스 주문을 합친 한국 시간 기준입니다.',
  },
  ...
  'coupang.experiments': {
    ...
    steps: [
      ...
      { title: '순위까지', desc: '[순위 추적]에 등록한 상품이면 키워드별 평균 순위의 변화도 같이 나옵니다.' },
    ],
    caution: '바꾼 지 3일은 지나야 판정이 나옵니다. 원가를 안 넣은 상품은 순이익이 원가 0으로 계산되니 [원가 입력]을 먼저 채우세요.',
  },
  ...
  'coupang.purchases': {
    ...
    steps: [
      ...
      { title: '여러 번 매입', desc: '같은 옵션을 다시 매입하면 총액 ÷ 총수량의 가중평균이 원가가 됩니다. 옵션별 표에서 현재 원가와 다르면 [원가에 반영]으로 맞춥니다.' },
    ],
    caution: '수입부가세는 일반과세자면 매입세액으로 돌려받는 돈이라 기본은 원가에서 뺍니다. 못 돌려받는 경우에만 "부가세를 원가에 포함"을 켜세요.',
  },
  ...
  'coupang.reconcile': {
    ...
    steps: [
      ...
      { title: '차이 읽기', desc: '차이 = 쿠팡 재고 − 예상 재고. 음수면 보낸 것보다 적은 것(미입고·분실·불량 반출), 양수면 더 많은 것(반품 재입고 등)입니다.' },
      { title: '언제부터 어긋났나', desc: '펼친 칸 아래의 30일 재고 흐름에서 판매량보다 재고가 더 줄어든 날을 찾으세요.' },
    ],
    caution: '기준을 잡기 전의 재고는 셈에 들어가지 않습니다. 반품이 재입고되면 예상보다 재고가 많아 보일 수 있습니다.',
  },
