 ...
 *
 * 예전에는 이 표가 여섯 벌 있었다(qa·generate-text·analyze-image·
 * generate-image·usage, 그리고 이 파일). 크기도 4개부터 15개까지 제각각이었고
 * 이미 갈라져 있었다 — gemini-2.5-flash가 어떤 파일에서는 0원이고 여기서는
 * 0.30이었다. 그래서 그 파일을 거친 호출은 원가가 0원으로 기록됐고, 관리자
 * 화면의 AI 원가가 실제보다 적게 나왔다. 한 곳에 둔다.
 *
 * 이미지 모델 단가는 기능을 내린 뒤에도 남겨 둔다. api_calls에 쌓인 지난
 * 호출이 그 모델 이름을 그대로 들고 있어서, 표에서 지우면 예전 원가가 전부
 * 0원으로 바뀐다 — 위에 적은 바로 그 사고다.
 *
