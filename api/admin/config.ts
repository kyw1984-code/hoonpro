...
const DEFAULTS = { imageModel: 'gpt-image-1.5', imageQuality: 'medium' };
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // 설정 변경
  if (req.method === 'POST') {
    const { imageModel, imageQuality } = req.body ?? {};
    ...
    return res.status(200).json({ imageModel, imageQuality, message: '저장됐습니다.' });
  }
