...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const { name, cohort, phone, email } = req.body ?? {};
  if (!name || !cohort || !phone || !email) {
    return res.status(400).json({ error: '이름, 라이브 기수, 연락처, 이메일을 모두 입력해주세요.' });
  }
  ...
  const { error } = await supabase.from('users').insert({ name, cohort, phone, email });
